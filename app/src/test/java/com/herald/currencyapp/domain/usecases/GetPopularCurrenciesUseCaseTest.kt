...
class GetPopularCurrenciesUseCaseTest {


    @Mock
    private lateinit var mockRetroRepo: RetroRepo

    private lateinit var getPopularCurrenciesUseCase: GetPopularCurrenciesUseCase

    @Before
    fun setup() {
        MockitoAnnotations.openMocks(this)
        getPopularCurrenciesUseCase = GetPopularCurrenciesUseCase(mockRetroRepo)
    }
    ...
    fun `invoke success`() = runBlocking {
        ...
        Mockito.`when`(mockRetroRepo.getExchangeRate(date = Constants.Latest_Date, from = fromCurrency, to =  Constants.Top_Currencies))
            .thenReturn(mockData)

        val result = getPopularCurrenciesUseCase(fromCurrency).toList()
